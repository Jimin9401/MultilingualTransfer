...
def load_dataset(args, tokenizer, task="nmt"):
    ...
    if not os.path.isdir(pairwise_cache):
        ...
        # pd.to_csv(dev, os.path.join(pairwise_cache, "dev.csv"))
        # pd.to_csv(test, os.path.join(pairwise_cache, "test.csv"))

        train_pairs = get_pairs_from_multilingual(train, src=args.src, trg=args.trg)
        train_examples = convert_data_to_examples(tokenizer, train_pairs)
        ...
        dev_examples = convert_data_to_examples(tokenizer, dev_pairs)
        ...
        test_examples = convert_data_to_examples(tokenizer, test_pairs)
...
def convert_data_to_examples(tokenizer: MBart50Tokenizer, dataset, type="train", replaced=False):
    examples = []
    for idx, (src, trg) in enumerate(zip(dataset["src"], dataset["trg"])):
        if replaced:
